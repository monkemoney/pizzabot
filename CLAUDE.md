...
- **Validated against the real model (2026-08-27, ~$2.00).** Three conversations through the real `buildSystemPrompt` + production Opus, with the US tenant config and the exempt category supplied in memory (menu read from Supabase read-only). All 16 assertions passed — a customer who wrote only "1200 Ocean Ave, LA area" was asked for the ZIP, and 90401 then resolved Santa Monica's $6.99 fee AND its 10.25% rate; a Hebrew-speaking customer at the LA business was asked for the מיקוד in Hebrew and quoted `סה"כ לפני מס`. Two findings the structural tests could not have caught:
  - ⚠️ **`taxable` in the products JOIN would have emptied every tenant's menu.** `menu-service` selected `categories(..., taxable)`, and a join naming a column the database does not have fails the WHOLE query — which `getProducts` turns into an empty menu. Deploying before the migration ran would have left every bot with no menu at all, not a missing flag. The column is read from the categories list (`select('*')`) instead, which simply comes back without the key; **reads no longer depend on the migration order at all**. Pinned by a test that mocks the pre-migration shape.
  - **The bot had never been told anything is exempt.** It names a flat rate ("Sales Tax 9.5% is added at checkout"), so an order with an exempt drink was quoted a tax $1.62 higher than the server charges — the same shape as the round-8 topping-price finding, in the customer's favour rather than against, but still a quote that does not match the charge. `taxRule(loc, lang, hasExempt)` now tells the bot the tax applies to taxable items only and never to state it as an amount. It is told the RULE, not asked to do the arithmetic: the ACTION total stays pre-tax and the server still owns the number. Gated on the tenant actually exempting something, so no existing prompt changes.
...
### US Address Model — the ZIP is the key (2026-08-27)

An address was one free-text string that a zone was resolved from by matching a city name inside it. That works for a Hebrew address, where the city name *is* the identifier. An American address is `street · city · state · ZIP`, and the ZIP is the part that names a jurisdiction — so city matching demos fine and then meets a customer who writes "LA", or writes only a street and a ZIP, and silently resolves their order to the wrong zone, the wrong fee and (since C9) the wrong tax rate.

- **`delivery_zones` gains an optional `zips`**, and `zoneForAddress()` matches on it FIRST. Most specific wins, exactly like the city sort: an exact ZIP beats a zone claiming the prefix `904`. **The branch only exists once a zone declares `zips`** — no tenant has one today, so every existing address still resolves through the city path, unchanged. An unclaimed ZIP falls *through* to the city match rather than failing, so a half-configured tenant keeps the answer it had.
- **`extractPostal()` is region-shaped, and that is the whole safety argument.** A bare `\d{5}` would bite the first seven-digit Israeli מיקוד it met and hand back five digits of it — and there is no useful difference between a wrong postal code and no postal code. `postal_re` lives in `REGIONS` (`\b\d{7}\b` vs `\b\d{5}(?:-\d{4})?\b`); ZIP+4 reduces to its base five, the unit a zone is configured in.
- **The bot asks for it.** `addressAsk(loc, lang)` in `prompt-en.js` returns the pre-existing line **byte-for-byte** for a region with no `subdivision`, so the frozen Hebrew prompt keeps meaning what it says, and a structured ask (street+number, city, state, ZIP — "the ZIP is required: it decides the delivery area and the tax rate", plus "the ACTION `address` must contain it") for a US tenant, in whichever language the customer is speaking. Address shape follows the TENANT; only the wording follows the customer.
- **The dashboard resolves a zone the same way the server does**, so `/business-config` now serves `postal_re`/`postal_label`/`address_order`/`subdivision` rather than the client re-deriving the rule — a second definition of one rule is failure class 10.
- **The order-edit modal's address fields were dead.** They read `order.address_city` / `address_street` / `address_num`, columns that have never existed, so the modal always opened blank and saving replaced a good address with whatever the operator retyped. `parseAddress()`/`composeAddress()` now split and rebuild the one stored string in the region's own order ("רוטשילד 5, תל אביב" vs "123 Main St, Los Angeles, CA 90012"), with state and ZIP fields shown only where an address has them. The round trip is verified to be **identity** — an address nobody edited must survive untouched — and anything the parser cannot place stays in `street` rather than being dropped.
- **Both zone editors carry it:** dashboard settings (US only, with the "most specific wins / prefix covers a range" note) and the onboarding wizard (English sessions only). Typed as `90401, 90402`, stored as an array; `normaliseZones()` — the same door that validates `tax_rate` — normalises it and **removes the key when emptied**, since a zone covering no postal code would never match anything again.
...
7. **delivery_zones** is authoritative (city, area, fee, min_order, eta_minutes, plus optional `tax_rate` and `zips` outside Israel); `saveZones()` syncs legacy `delivery_cities`; bot reads zones first. `zoneForAddress()` is the ONE matcher — ZIP first where configured, then longest city name; fee and tax must resolve from the same row, or one address has two authorities
